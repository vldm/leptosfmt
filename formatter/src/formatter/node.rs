...
impl Formatter {
    pub fn node(&mut self, node: &Node) {
        match node {
            ...
            Node::RawText(text) => self.raw_text(text),
    ...
    pub fn raw_text(&mut self, text: &RawText) {
        let text = text.to_source_text(true).expect("Cannot format unquoted text, no source text available, or unquoted text is used outside of element.");
